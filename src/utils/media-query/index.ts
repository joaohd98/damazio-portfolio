import { aspectRatioBase, AspectRatioProps, ViewportBaseProps, viewportsBase } from '@/styles/viewport-base';
...
export const mediaMinWidth = <Props>(value: ViewportBaseProps | number, plus = 0) =>
  interpolation<Props>(`(min-width: ${viewPortOrPx(value) + plus}px)`);
export const mediaMaxWidth = <Props>(value: ViewportBaseProps | number, plus = 0) =>
  interpolation<Props>(`(max-width: ${viewPortOrPx(value) + plus}px)`);
...
export const mediaMinAspectRatio = (key: AspectRatioProps) =>
  interpolation(`(min-aspect-ratio: ${aspectRatioBase[key]})`);
